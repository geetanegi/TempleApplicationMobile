...
const TeeCartCheckout = ({courceData, onPressList, courceName}) => {
      ...
      <View
          ...
          <View>
            {contest &&
              contest?.map(val => {
                ...
                const startDate = moment(val?.startTime).format('MMM DD');
                const endDate = moment(val?.endTime).format('MMM DD');
                  ...
                  <View style={styles.metricBoxInside}>
                    ...
                    <View style={styles.itemDetails}>
                      ...
                      <View style={styles.itemSubDetails}>
                        ...
                        {/* <Text style={styles.dateRange}>90-00</Text> */}
                          <FontAwesome5
                            name="golf-ball"
                            size={13}
        ...
        {expanded && (
          ...
          <View
            ...
            <TouchableOpacity activeOpacity={0.8} style={styles.chectoutButton}  onPress={() => onPressList(selectedItems)} >
              {selectedItems != '' && (
  ...
  const toggleSelect = (item) => {
    ...
    if (isSelected) {
      ...
      setSelectedItems(selectedItems.filter(contestId => contestId !== item.contestId));
      setCheckOutTotal(selectedItems.filter(contestId => contestId !== item.contestId));
      ...
      // Otherwise, add the item to selected items
       ...
       const toggleSelect = item => {
        ...
        if (isSelected) {
          ...
          // Otherwise, add the item to selected items
           setSelectedItems([...selectedItems, item.contestId]);
          setCheckOutTotal([...checkOutTotal, item.entryFee]);
        }
      };
    }
  };
